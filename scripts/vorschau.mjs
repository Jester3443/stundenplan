...
if (!tag || !tag.stunden.length) {
  console.log(`${zieldatum}: keine Stunden im Plan - nichts zu melden.`);
  process.exit(0);
}
...
// Hausaufgaben, die Lehrkraefte fuer diesen Tag eingetragen haben.
...
if (!titel && !hausaufgaben.length) {
  console.log(`${zieldatum}: alles nach Plan, keine Aufgaben - keine Nachricht.`);
  process.exit(0);
}
...
console.log(
  process.env.KNAPP
    ? `${ART}: ${verschickt} Nachricht(en) verschickt.`
    : `${ART}: ${verschickt} Nachricht(en) verschickt - "${titel}"`
);
